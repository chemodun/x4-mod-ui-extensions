...
ffi.cdef[[
	...
	bool GetButtonBorderColor(const int buttonid, GlowColor* color);
	...
	DropDownBackgroundInfo GetDropDownBackground2Details(const int dropdownid);
	bool GetDropDownBorderColor(const int dropdownid, GlowColor* color);
	bool GetDropDownColor(const int dropdownid, Color* color);
	DropDownInfo GetDropDownDetails(const int dropdownid);
	...
	uint32_t GetDropDownOptionOverlayInfo2(UIOverlayInfo3* result, uint32_t resultlen, const int dropdownid);
	uint32_t GetDropDownOptions3(DropDownOption3* result, uint32_t resultlen, const int dropdownid);
	const char* GetDropDownStartOption(const int dropdownid);
	bool GetDropDownTextProperties(const int dropdownid, uint32_t, DropDownTextInfo* textinfo);
	...
	uint32_t GetFrameBorders(UIFrameBorderInfo* result, uint32_t resultlen, const int frameid);
	...
	bool GetGraphXAxis2(const int graphid, GraphAxisInfo2* axis);
	bool GetGraphYAxis2(const int graphid, GraphAxisInfo2* axis);
	GraphInitialSelectionInfo GetGraphInitialSelectionInfo(const int graphid);
	UIOverlayInfo3 GetHelpOverlayInfo3(const int widgetid);
	...
	uint32_t GetNumFrameBorders(const int frameid);
	...
	uint32_t GetNumTableRowGroups(const int tableid);
	uint32_t GetRenderTargetAlpha(const int rendertargetid);
	const char* GetRenderTargetFrameBorderID(int rendertargetid);
	...
	bool GetSlidercellBorderColor(const int slidercellid, GlowColor* color);
	...
	UIOverlayInfo3 GetStandardButtonHelpOverlayInfo3(const int frameid, const char* button);
	...
	const char* GetTableFrameBorderID(const int tableid);
	...
	uint32_t GetTableRowGroups(RowGroupInfo* result, uint32_t resultlen, const int tableid);
	uint32_t GetTableRowInfo(TableRowInfo* result, uint32_t resultlen, const int tableid);
	PaddingInfo GetTableRowPadding(const int tableid, uint32_t row);
...
config.texturesizes = {
	...
	table = {
		scrollBar = {
			...
			sliderCenterElementWidth  = 4,  -- height of the center part of the slider element in the scrollbar
			...
			border                    = 4,  -- width of the scrollbar
		}
	}
}
...
local private = {
	...
	element = {		-- the visible Anark elements
		...
		tableRowGroups = {
			-- [rowgroupindex] = {
				-- element					= row group anark element
			-- }
		},
		table = {
			-- [tableindex] = {
		...
		frameborder = {
			-- [frameborderindex] = frameBorderElement
		},
		icon = {
	...
	fadingButtonHighlights = {},
	helpOverlays = {},
	displayedHelpOverlays = {},
	signalledHelpOverlays = {},
	helpOverlayStates = {},
	uiscale = 1.0,
	inputBar = nil,                   -- indicates an active input bar and stores its properties
	-- {
		-- height                     -- the total height of the mouse over text
		-- inputsLeft
		-- inputsRight
...
function self:onInitialize()
	...
	widgetSystem.initializeFrameBorderElements()
	...
	widgetSystem.initializeTableRowGroupElements()
...
function self:onUpdate()
	...
	if #private.pendingFrames > 0 then
			...
			if not skip then
				...
				if (private.activeframe ~= 0) and IsValidWidgetElement(private.activeframe) then
					...
					if interactiveWidgetID ~= nil then
						if interactiveWidgetID == (private.interactiveElement and private.interactiveElement.widgetID) then
							-- don't clear interactiveElement, but we still need to init things
							widgetSystem.initTableInputHighlight(interactiveWidgetID, tableElement)
						else
		...
		private.panelMode = C.IsInPanelMode()
		...
		if private.inputBar then
			local inputBarElement = getElement("inputBar", private.widgetsystem)
			goToSlide(inputBarElement, "active")
			widgetSystem.updateInputBar(private.inputBar.inputsLeft, private.inputBar.inputsRight)
		end
	...
	-- animate button highlightcolors
	curTime = curTime or getElapsedTime()
	for buttonID, starttime in pairs(private.fadingButtonHighlights) do
		local buttonentry = private.associationList[buttonID]
		if buttonentry == nil then
			-- element doesn't exist anymore
			private.fadingButtonHighlights[buttonID] = nil
		else
			local buttonElement = buttonentry.element

			local t = math.min(1, (curTime - starttime) / config.button.highlightFadeDuration)
			local color = {
				r = t * buttonElement.color.r + (1 - t) * buttonElement.highlightcolor.r,
				g = t * buttonElement.color.g + (1 - t) * buttonElement.highlightcolor.g,
				b = t * buttonElement.color.b + (1 - t) * buttonElement.highlightcolor.b,
				a = t * buttonElement.color.a + (1 - t) * buttonElement.highlightcolor.a,
				glow = t * buttonElement.color.glow + (1 - t) * buttonElement.highlightcolor.glow,
			}
			...
			setAttribute(material, "glowfactor", color.glow or 0)

			if t >= 1 then
				-- done with the fade, stop animation
				private.fadingButtonHighlights[buttonID] = nil
			end
		end
	end
...
function widgetSystem.onBack()
	...
	elseif private.activeframe then
		...
		for frameidx = 0, #frameElements do
			...
			if frameElement and frameElement.frame then
				widgetSystem.raiseHideEvent(frameElement.frame, "back")
			end
		end
	end
end
	...
	elseif private.activeDropDown then
		...
		for i, entry in ipairs(private.activeDropDown.element.options) do
			if (not entry.deleted) and entry.active then
				local lowertext = utf8.lower(entry.text1)
				...
				if utf8.find(lowertext, private.activeDropDown.directInputString, 1, true) == 1 then
					...
					if private.activeDropDown.selected ~= private.activeDropDown.highlighted then
						...
						widgetSystem.updateDropDownOptionState(private.activeDropDown.widgetID, private.activeDropDown.element, oldhighlighted)
					end
...
function widgetSystem.onDropDownUpdate(_, dropdownID)
	...
	local buf = ffi.new("DropDownOption3[?]", n)
	n = C.GetDropDownOptions3(buf, n, dropdownID)
	for i = 0, n - 1 do
		for j = 1, config.dropdown.numtexts do
			dropdownElement.options[i + 1]["text" .. j] = ffi.string(buf[i]["text" .. j])
		end
	end

	local isactive = widgetSystem.isWidgetActive(dropdownID) and C.IsDropDownActive(dropdownID)
...
function widgetSystem.onEditBoxUpdate(_, editboxID)
	...
	local isactive = widgetSystem.isWidgetActive(editboxID) and C.IsEditBoxActive(editboxID)
...
function widgetSystem.onMouseClickBoxText(anarkElement, delayed, modified)
	if delayed then
		return -- ignore delayed clicks
	end

	local boxTextElement, frameElement = widgetSystem.getTableCellElementByAnarkElement("boxtext", anarkElement)
...
function widgetSystem.onRightMouseClickButton(anarkElement, delayed)
	...
	local cellposx = tableElement.cellposx[col] + cellwidth + private.scaledSizes.table_borderSize
...
function widgetSystem.onMouseClickPageScroll(anarkElement, delayed)
	...
	if not tableElement.scrollBar.readonly then
		if y > sliderPosY then
...
function widgetSystem.onMouseOverGraphDataPoint(anarkElement)
	...
	if graphElement.xAxis.unittext and (graphElement.xAxis.unittext ~= "") then
		textx = textx .. " " .. graphElement.xAxis.unittext
	end
	...
	if graphElement.yAxis.unittext and (graphElement.yAxis.unittext ~= "") then
		texty = texty .. " " .. graphElement.yAxis.unittext
	end
	local graphID = widgetSystem.getWidgetIDByElementEntry(graphElement)
...
function widgetSystem.onStopSelect(_, widgetID)
	...
	if IsType(widgetID, "button") then
		-- hotkey case
		if widgetSystem.isWidgetActive(buttonID) and IsButtonActive(widgetID) then
	...
	elseif IsType(widgetID, "dropdown") then
		-- hotkey case
		if widgetSystem.isWidgetActive(dropdownID) and C.IsDropDownActive(widgetID) then
		...
		if IsType(widgetID, "table") and private.interactiveElement.element.interactiveChild ~= nil then
			...
			if interactiveElement ~= nil then
				...
				elseif IsType(interactiveWidgetID, "dropdown") then
					...
					if interactiveElement.active then
						widgetID = interactiveWidgetID
...
function widgetSystem.onStopInteract(_, widgetID)
	...
	if IsType(widgetID, "table") then
		...
		if tableElement.interactiveChild ~= nil then
			...
			local cellposx = tableElement.cellposx[child.col] + cellwidth + private.scaledSizes.table_borderSize
		...
		if row then
			cellposx = tableElement.cellposx[1] + tableElement.width + private.scaledSizes.table_borderSize
...
function widgetSystem.calculateRowsToMoveByBottomRow(tableID, tableElement, newBottomRow)
	...
	for row = newBottomRow, newTopRow, -1 do
		...
		if row ~= newBottomRow then
			...
			if tableElement.borderbelowrows[row] then
				nextrowheight = nextrowheight + private.scaledSizes.table_borderSize
			end
			nextrowheight = nextrowheight + tableElement.paddingrows[row].top + tableElement.paddingrows[row].bottom
		end

		if tableElement.rowgrouprows[row] ~= prevrowgroup then
			prevrowgroup = tableElement.rowgrouprows[row]
			nextrowheight = nextrowheight + private.scaledSizes.tableRowGroups_borderSize
		end
...
function widgetSystem.calculateFixedRowHeight(tableID, tableElement)
	...
	if numFixedRows > 0 then
		...
		for row = 1, numFixedRows do
			...
			minRowHeight = minRowHeight + tableElement.paddingrows[row].top + tableElement.paddingrows[row].bottom
		end

		for _, rowgroup in pairs(tableElement.rowGroups) do
			if rowgroup.firstrow <= numFixedRows then
				minRowHeight = minRowHeight + 2 * private.scaledSizes.tableRowGroups_borderSize
			end
		end
		...
		minRowHeight = minRowHeight + numBorderElements * private.scaledSizes.table_borderSize
	end
...
function widgetSystem.calculateMinRowHeight(tableID, tableElement)
	...
	for row = startRow, numRows do
		...
		if tableElement.borderbelowrows[row] then
			curBorderHeight = curBorderHeight + private.scaledSizes.table_borderSize
		end
		curBorderHeight = curBorderHeight + tableElement.paddingrows[row].top + tableElement.paddingrows[row].bottom
	end
...
function widgetSystem.getColumnSpanInfo(tableID, tableElement, row, col, isFixedRowSection)
	...
	local rowgroupid = tableElement.rowgrouprows[row]
	local rowgroup
	if rowgroupid ~= 0 then
		rowgroup = tableElement.rowGroups[rowgroupid]
	end

	local cellwidth = 0
	for i = 1, colspan do
		local spannedcol = col + i - 1

		if isFixedRowSection then
			cellwidth = cellwidth + tableElement.fixedRowColumnWidths[spannedcol]
		else
			cellwidth = cellwidth + tableElement.columnWidths[spannedcol]
		end
	...
	cellwidth = cellwidth + private.scaledSizes.table_borderSize * (colspan - 1)
...
function widgetSystem.getBackgroundColumnSpanInfo(tableID, tableElement, row, col, isFixedRowSection)
	...
	cellwidth = cellwidth + private.scaledSizes.table_borderSize * (backgroundcolspan - 1)
...
function widgetSystem.drawTableSection(frameElement, tableID, tableElement, firstRow, lastRow, displayedRows, cellposy, curtableheight, cellIndexOffset, isFixedRowSection, firsttime)
	for row = firstRow, lastRow do
		...
		if (curtableheight ~= 0) then
			...
			local prevrow = ((row == firstRow) and (not isFixedRowSection)) and tableElement.numFixedRows or (row - 1)
			if tableElement.borderbelowrows[prevrow] then
				nextrowheight = nextrowheight + private.scaledSizes.table_borderSize
				cellposy = cellposy - private.scaledSizes.table_borderSize
			end
		end

		-- check whether a row group-ended and add extra space
		local currentgroup, previousgroup
		local currentgroupid = tableElement.rowgrouprows[row]
		if currentgroupid ~= 0 then
			currentgroup = tableElement.rowGroups[currentgroupid]
		end
		if (row ~= firstRow) or (row > tableElement.numFixedRows) then
			local prevrowindex = row - 1
			if row == firstRow then
				-- first row after fixed rows, check that row for a previous row group
				prevrowindex = tableElement.numFixedRows
			end

			local previousgroupid = tableElement.rowgrouprows[prevrowindex]
			if previousgroupid ~= 0 then
				previousgroup = tableElement.rowGroups[previousgroupid]
			end

			local previouslevel = previousgroup and previousgroup.level or 0
			local currentlevel = currentgroup and currentgroup.level or 0
			if row == firstRow then
				-- switching from fixed rows to non-fixed rows -> terminate all groups
				currentlevel = 0
			end

			if previouslevel > currentlevel then
				cellposy = cellposy - (previouslevel - currentlevel) * private.scaledSizes.tableRowGroups_borderSize
				curtableheight = curtableheight + (previouslevel - currentlevel) * private.scaledSizes.tableRowGroups_borderSize
			end
		end
		...
		if curtableheight + nextrowheight > tableElement.height then
			...
			if previouslevel > 0 then
				-- make space to terminate all rowgroups
				curtableheight = curtableheight + previouslevel * private.scaledSizes.tableRowGroups_borderSize
			end
		...
		curtableheight = curtableheight + nextrowheight + tableElement.paddingrows[row].top + tableElement.paddingrows[row].bottom
		...
		cellposy = cellposy - curRowHeight / 2 - tableElement.paddingrows[row].top

		tableElement.cell[row - firstRow + 1 + cellIndexOffset].realRow = row

		-- place row group elements
		tableElement.cell[row - firstRow + 1 + cellIndexOffset].rowGroups = {}
		local rowgroups = tableElement.cell[row - firstRow + 1 + cellIndexOffset].rowGroups
		if row == firstRow then
			-- check if any row groups are supposed to be already visible
			for _, rowgroup in pairs(tableElement.rowGroups) do
				if (rowgroup.firstrow <= row) and ((rowgroup.firstrow + rowgroup.numrows) > row) then
					local overridenumrows = rowgroup.firstrow + rowgroup.numrows - row

					local rowgroupelement = widgetSystem.getElement("tableRowGroups")
					if rowgroupelement ~= nil then
						widgetSystem.moveAnarkElementFrameLayer(frameElement, rowgroupelement.element, 1)
						widgetSystem.setUpRowGroup(rowgroup.groupid, rowgroupelement, tableElement.cellposx[1], cellposy + curRowHeight / 2, overridenumrows, tableID, tableElement, curtableheight - curRowHeight, isFixedRowSection)
						table.insert(rowgroups, rowgroupelement)

						cellposy = cellposy - private.scaledSizes.tableRowGroups_borderSize
						curtableheight = curtableheight + private.scaledSizes.tableRowGroups_borderSize
					else
						DebugError("Widget system error. Already displaying " .. config.tableRowGroups.maxRowGroups .. " row groups. Cannot display more. Row group in table row " .. rowgroup.firstrow .. " will be skipped.")
					end
				end
			end
		else
			-- check whether a row group starts in this row
			local rowgroup = tableElement.rowGroupsByFirstRow[row]
			if rowgroup then
				local rowgroupelement = widgetSystem.getElement("tableRowGroups")
				if rowgroupelement ~= nil then
					widgetSystem.moveAnarkElementFrameLayer(frameElement, rowgroupelement.element, 1)
					widgetSystem.setUpRowGroup(rowgroup.groupid, rowgroupelement, tableElement.cellposx[1], cellposy + curRowHeight / 2, nil, tableID, tableElement, curtableheight - curRowHeight, isFixedRowSection)
					table.insert(rowgroups, rowgroupelement)
		...
		for col = 1, tableElement.numCols do
			...
			if colspan ~= 0 then
				...
				if (col + colspan <= tableElement.numCols) and (GetTableBackgroundColumnSpan(tableID, row, col + colspan) == 0) then
					...
					cellbackgroundwidth = cellwidth + private.scaledSizes.table_borderSize
					isColBGSpanned = true
				end
				...
				local childWidgetID = GetCellContent(tableID, row, col)
				if tableElement.frameBorderID ~= "" then
					private.panelAssociation[childWidgetID] = tableElement.frameBorderID
				end
				...
				elseif IsType(childWidgetID, "graph") then
					...
					if graphElement ~= nil then
						...
						-- Helper.printDebugValue(cellentry.graph, "CellG")
					else
						if private.miniWidgetSystemUsed then
				...
				elseif IsType(childWidgetID, "slidercell") then
					...
					if slidercellElement ~= nil then
						...
						widgetSystem.setUpSliderCell(frameElement, childWidgetID, cellentry.slidercell, cellposx + cellwidth / 2, cellposy + curRowHeight / 2, cellwidth, curRowHeight, backgroundcolspanwidth)
					else
						if private.miniWidgetSystemUsed then
		...
		cellposy = cellposy - curRowHeight/2 - tableElement.paddingrows[row].bottom

		if row == lastRow then
			local currentlevel = currentgroup and currentgroup.level or 0
			if currentlevel > 0 then
				-- make space to terminate all rowgroups
				curtableheight = curtableheight + currentlevel * private.scaledSizes.tableRowGroups_borderSize
			end
		end
...
function widgetSystem.drawFlowchartContent(frameElement, flowchartID, flowchartElement)
	...
	local colheight = flowchartData.maxVisibleHeight + 2 * private.scaledSizes.table_borderSize
...
function widgetSystem.drawFlowchartEdge(frameElement, flowchartEdgeID, flowchartEdgeElement, edge, flowchartData)
	...
	x0, y0, x1, y1 = ClipLine(x0, y0, x1, y1, xmin, ymin - private.scaledSizes.table_borderSize, xmax, ymax + private.scaledSizes.table_borderSize)
	if not x0 then
		-- line is not in view
		return false
	end
...
function widgetSystem.getDropDownOptionByOptionAnarkElement(dropdownElement, optionElement)
	for i, option in ipairs(dropdownElement.options) do
		if option.element == optionElement then
			return { option.mouseovertext, (option.text1 ~= option.truncatedtext1) and option.text1 or nil }
		end
	end
end
		...
		if frameElement then
			for _, tableElement in ipairs(frameElement.tables) do
				if tableElement.scrollBar.element == scrollBarElement then
					return tableElement, frameElement, tableElement.scrollBar
				end
			end
		end
	end
...
function widgetSystem.getUsableTableWidth(width, offsetx, numColumns, hasScrollBar)
	...
	local usablewidth = width - offsetx - (numColumns - 1) * private.scaledSizes.table_borderSize
	if hasScrollBar then
		usablewidth = usablewidth - private.scaledSizes.scrollBar_width - private.scaledSizes.scrollBar_borderSize
	end
	return usablewidth
end
...
function widgetSystem.hideRowGroup(groupElement)
	setAttribute(getElement("background", groupElement.element), "position.z", config.tableRowGroups.baseZValue)
	goToSlide(groupElement.element, "inactive")
	widgetSystem.returnElement("tableRowGroups", groupElement)
end
...
function widgetSystem.hideDropDown(dropdownElement, iconActive, hotykeyIconActive)
	...
	for i = 1, config.dropdown.numtexts do
		setAttribute(getElement("Text" .. i, dropdownElement.element.mainElement), "textstring", "")
		dropdownElement["text" .. i .. "Active"] = nil
	end
	if hotkeyIconActive then
...
function widgetSystem.hideFrame(frameElement, keepMinimizedState)
	...
	for _, frameborder in pairs(frameElement.frameborders) do
		if frameborder.element then
			widgetSystem.moveAnarkElementFrameLayer(frameElement, frameborder.element, -1)
			widgetSystem.hideFrameElementBorder(frameborder.element)
		end
	end
...
function widgetSystem.hideTableCells(frameElement, tableElement)
	...
	for row = 1, displayedRows do
		...
		for _, groupelement in ipairs(rowarray.rowGroups) do
			widgetSystem.moveAnarkElementFrameLayer(frameElement, groupelement.element, -1)
			widgetSystem.hideRowGroup(groupelement)
		end
		rowarray.rowGroups = {}
...
function widgetSystem.highlightBorderTableRow(tableID, tableElement, row, col)
	...
	elseif tableElement.highlightMode == "backgroundcolumn" then
		...
		if rowgroup then
			width = width - 2 * rowgroup.level * private.scaledSizes.tableRowGroups_borderSize
		end
	end
	local height = GetTableRowHeight(tableID, row)

	-- position of the upper left corner of the table row
	local offsetx, offsety = tableElement.cellposx[col], tableElement.cellposy[row]
	if rowgroup then
		if col == 1 then
			offsetx = offsetx + rowgroup.level * private.scaledSizes.tableRowGroups_borderSize
		end
	end
	...
	local highlightborderwidth = private.scaledSizes.table_highlightBorderSize
	...
	widgetSystem.setElementPositionUnchecked(tableElement.borderElements.top,    offsetx + (width - highlightborderwidth) / 2, offsety + (height + highlightborderwidth) / 2)
	widgetSystem.setElementPositionUnchecked(tableElement.borderElements.bottom, offsetx + (width + highlightborderwidth) / 2, offsety - (height + highlightborderwidth) / 2)
	widgetSystem.setElementPositionUnchecked(tableElement.borderElements.left,   offsetx -          highlightborderwidth / 2,  offsety           - highlightborderwidth / 2)
	widgetSystem.setElementPositionUnchecked(tableElement.borderElements.right,  offsetx + width  + highlightborderwidth / 2,  offsety           + highlightborderwidth / 2)

	-- scale border elements
	widgetSystem.setElementScale(tableElement.borderElements.top,    (width + highlightborderwidth) / 100,           highlightborderwidth / 100)
	widgetSystem.setElementScale(tableElement.borderElements.bottom, (width + highlightborderwidth) / 100,           highlightborderwidth / 100)
	widgetSystem.setElementScale(tableElement.borderElements.left,            highlightborderwidth / 100,  (height + highlightborderwidth) / 100)
	widgetSystem.setElementScale(tableElement.borderElements.right,           highlightborderwidth / 100,  (height + highlightborderwidth) / 100)
end
...
function widgetSystem.removeHighlightBorderTableRow(tableElement)
	...
	if private.enableMouseOverText then
		if private.hadNoneMouseInput then
			if private.mouseOverText and (((GetControllerInfo() == "gamepad") and (not C.IsMouseEmulationActive())) or (private.mouseOverText.posOverride ~= nil)) then
				if not private.gamepadInteractiveChildMouseOver then
					private.gamepadTableMouseOver = nil
					widgetSystem.hideMouseOverText(private.mouseOverText.widgetID)
				end
			end
...
function widgetSystem.initializeMasterElements()
	...
	private.master.frameBorder           = getElement("frameElementBorder", layerElement)
	...
	tablearray.rowGroup  = getElement("table_rowgroup", layerElement)
...
function widgetSystem.prepareSize()
	...
	private.scaledSizes = {
		table_borderSize = widgetSystem.scaleSizeMinValue(config.table.bordersize, 2),
		table_highlightBorderSize = widgetSystem.scaleSizeMinValue(config.table.highlightBorderSize, 1),
		tableRowGroups_borderSize = widgetSystem.scaleSizeMinValue(config.tableRowGroups.borderSize, 3),
		scrollBar_width = widgetSystem.scaleSizeMinValue(config.texturesizes.table.scrollBar.width, 3),
		scrollBar_sliderWidth = widgetSystem.scaleSizeMinValue(config.texturesizes.table.scrollBar.sliderCenterElementWidth, 1),
		scrollBar_borderSize = widgetSystem.scaleSizeMinValue(config.texturesizes.table.scrollBar.border, 3),
		button_border = widgetSystem.scaleSizeMinValue(config.button.border, 1),
		checkbox_border = widgetSystem.scaleSizeMinValue(config.checkbox.border, 2),
		dropdown_border = widgetSystem.scaleSizeMinValue(config.dropdown.borderSize, 1),
		slidercell_border = widgetSystem.scaleSizeMinValue(config.slidercell.border, 1),
		editbox_border = widgetSystem.scaleSizeMinValue(config.editbox.border, 2),
	}
	-- make sure the table_highlightBorderSize is one px smaller than table_borderSize
	if private.scaledSizes.table_highlightBorderSize >= private.scaledSizes.table_borderSize then
		private.scaledSizes.table_highlightBorderSize = private.scaledSizes.table_borderSize - 1
	end
	-- make sure the difference between scrollBar_width and scrollBar_sliderWidth is an even number
	if (private.scaledSizes.scrollBar_width - private.scaledSizes.scrollBar_sliderWidth) % 2 == 1 then
		private.scaledSizes.scrollBar_sliderWidth = private.scaledSizes.scrollBar_sliderWidth - 1
	end
end

function widgetSystem.scaleSizeMinValue(insize, minvalue)
	return math.max(minvalue, math.floor(insize * private.uiscale + 0.5))
end
...
function widgetSystem.initializeTableRowGroupElements()
	for rowgroupindex = 1, config.tableRowGroups.maxRowGroups do
		private.element.tableRowGroups[rowgroupindex] = {}
		local rowGroupElement = private.element.tableRowGroups[rowgroupindex]
		rowGroupElement.element = clone(private.master.table.rowGroup, "tablerowgroup" .. rowgroupindex)
	end
end
...
function widgetSystem.setDropDownRemoveOptionColor(dropdownElement, removeOptionElement, colorMode)
	local r, g, b, a = dropdownElement.text1Details.color.red, dropdownElement.text1Details.color.green, dropdownElement.text1Details.color.blue, dropdownElement.text1Details.color.alpha
	...
	setAttribute(material, "glowfactor", dropdownElement.text1Details.glowfactor)
...
function widgetSystem.setInteractiveElement(frameID, widgetID, allowregionchange, checked)
	...
	local tableElement = private.associationList[widgetID].element
	if not widgetSystem.isPanelActive(tableElement.frameBorderID) then
		return
	end
	...
	widgetSystem.initTableInputHighlight(widgetID, tableElement)
...
function widgetSystem.initTableInputHighlight(tableID, tableElement)
	if IsType(tableID, "table") then
		local tableElement = private.interactiveElement.element
		...
		if isNormalRegion then
			...
			elseif tableElement.highlightMode == "off" then
				if tableElement.scrollBar.active then
					widgetSystem.setScrollBarState(tableElement.scrollBar, "gamepadFocus", true, tableID)
				end
			end
		end
	end
...
function widgetSystem.setInteractiveChildElement(tableID, tableElement, row, col, childWidgetID, childTableElement)
	...
	if childTableElement ~= nil and private.interactiveElement ~= nil and private.interactiveElement.widgetID == tableID then
		...
		if private.enableMouseOverText then
			if ((GetControllerInfo() == "gamepad") and (not C.IsMouseEmulationActive())) or (private.mouseOverText and (private.mouseOverText.posOverride ~= nil)) then
				if ffi.string(C.GetMouseOverText(childWidgetID)) ~= "" then
					private.gamepadInteractiveChildMouseOver = true
					local isNormalRegion = (row > tableElement.numFixedRows)
					local cellposx = tableElement.cellposx[1] + tableElement.width
...
function widgetSystem.updateSliderCellBackground(slidercellElement)
	if slidercellElement.fromcenter then
		...
		local width = inactive.width
		if not slidercellElement.readonly then
			width = width - 2 * private.scaledSizes.slidercell_border
		end
		widgetSystem.setElementScale(inactiveElement, width / 100, height / 100)
...
function widgetSystem.initializeBoxTextElements()
	for count = 1, config.boxtext.maxElements do
		...
		-- register for mouse interactions
		registerForEvent("onMouseClick",       boxtextElement.mainElement, widgetSystem.onMouseClickBoxText)
...
function widgetSystem.initializeFrameBorderElements()
	for count = 1, config.frameElementBorder.maxElements do
		local frameBorderElement = clone(private.master.frameBorder, "frameborder"..count)
		table.insert(private.element.frameborder, frameBorderElement)
	end
end
	...
	for widgetID, entry in pairs(private.associationList) do
		if entry.element == element then
			if IsValidWidgetElement(widgetID) then
				local overlayid = ffi.string(C.GetHelpOverlayInfo3(widgetID).id)
...
function widgetSystem.setUpRowGroup(groupID, groupElement, x, y, numrowsoverride, tableID, tableElement, curtableheight, isFixedRowSection)
	local rowgroup = tableElement.rowGroups[groupID]
	local firstrow = rowgroup.firstrow
	local numrows = rowgroup.numrows
	if numrowsoverride then
		firstrow = firstrow + (numrows - numrowsoverride)
		numrows = numrowsoverride
	end

	local height = 0
	for row = firstrow, firstrow + numrows - 1 do
		local rowheight = GetTableRowHeight(tableID, row) + tableElement.paddingrows[row].top
		if curtableheight + height + rowheight > tableElement.height then
			-- not the whole row group fits, abort here
			if tableElement.borderbelowrows[row - 1] then
				height = height - private.scaledSizes.table_borderSize
			end
			break
		elseif row ~= firstrow + numrows - 1 then
			if tableElement.borderbelowrows[row] then
				rowheight = rowheight + private.scaledSizes.table_borderSize
			end
			rowheight = rowheight + tableElement.paddingrows[row].bottom
		end
		height = height + rowheight

		if row ~= firstrow then
			local previousgroup, currentgroup
			local previousgroupid = tableElement.rowgrouprows[row - 1]
			if previousgroupid ~= 0 then
				previousgroup = tableElement.rowGroups[previousgroupid]
			end
			local currentgroupid = tableElement.rowgrouprows[row]
			...
			-- check whether we changed groups and need to add some extra space
			if previouslevel ~= currentlevel then
				height = height + private.scaledSizes.tableRowGroups_borderSize
			end
			-- check whether multiple
			if (row == firstrow + numrows - 1) and (currentgroupid ~= groupID) then
				height = height + (currentlevel - rowgroup.level) * private.scaledSizes.tableRowGroups_borderSize
			end
		end
	end
	height = height + 2 * private.scaledSizes.tableRowGroups_borderSize
	local width = isFixedRowSection and tableElement.fixedrowwidth or tableElement.width

	goToSlide(groupElement.element, "background")

	widgetSystem.setElementPosition(groupElement.element, x + width / 2, y - height / 2, (width % 2 ~= 0), (height % 2 ~= 0))
	local backgroundElement = getElement("background", groupElement.element)
	setAttribute(backgroundElement, "position.z", config.tableRowGroups.baseZValue - 0.001 * (rowgroup.level - 1))
	widgetSystem.setElementScale(backgroundElement, (width - (2 * (rowgroup.level - 1) * private.scaledSizes.tableRowGroups_borderSize)) / 100, height / 100)

	local material = getElement("background.material", groupElement.element)
	SetDiffuseColor(material, rowgroup.color.r, rowgroup.color.g, rowgroup.color.b)
	setAttribute(material, "opacity", rowgroup.color.a)
	setAttribute(material, "glowfactor", rowgroup.color.glow)
end
...
function widgetSystem.setUpButton(frameElement, buttonID, buttonElement, isSelected, parentx, parenty, parentwidth, parentheight, backgroundcolspanwidth)
	buttonElement.active = widgetSystem.isWidgetActive(buttonID) and IsButtonActive(buttonID)
	...
	-- border
	widgetSystem.setElementScale(getElement("background", buttonElement.element), (width - 2 * private.scaledSizes.button_border) / 100, (height - 2 * private.scaledSizes.button_border) / 100)
...
function widgetSystem.setUpCheckBox(frameElement, checkboxID, checkboxElement, isSelected, parentx, parenty, parentwidth, parentheight, backgroundcolspanwidth)
	checkboxElement.active = widgetSystem.isWidgetActive(checkboxID) and C.IsCheckBoxActive(checkboxID)
	...
	-- background_black
	widgetSystem.setElementScale(getElement("background_black", checkboxElement.element), (width - 2 * private.scaledSizes.checkbox_border) / 100, (height - 2 * private.scaledSizes.checkbox_border) / 100)
...
function widgetSystem.setUpDropDown(frameElement, dropdownID, dropdownElement, isSelected, parentx, parenty, parentwidth, parentheight, backgroundcolspanwidth)
	local info = C.GetDropDownDetails(dropdownID)

	local startOption = ffi.string(info.startoption)
	dropdownElement.active = widgetSystem.isWidgetActive(dropdownID) and info.active
	dropdownElement.previouslyActive = false
	dropdownElement.allowMouseOverInteraction = info.mouseoverinteraction
	dropdownElement.rightSideArrow = info.rightsidearrow
	...
	local optionwidth = info.optionwidth
	if optionwidth == 0 then
		optionwidth = width
	end
	local optionheight = info.optionheight
	...
	-- border
	widgetSystem.setElementScale(getElement("background", dropdownElement.element.mainElement), (width - 2 * private.scaledSizes.dropdown_border) / 100, (height - 2 * private.scaledSizes.dropdown_border) / 100)

	dropdownElement.bordercolor = dropdownElement.active and Color["dropdown_border_default"] or Color["dropdown_border_inactive"]
	if dropdownElement.active then
		local color = ffi.new("GlowColor")
		if C.GetDropDownBorderColor(dropdownID, color) then
			dropdownElement.bordercolor = {
				["r"] = color.red,
				["g"] = color.green,
				["b"] = color.blue,
				["a"] = color.alpha,
	...
	-- background 2
	local background2element = getElement("background2", dropdownElement.element.mainElement)
	local background2material = getElement("Material753", background2element)
	if dropdownElement.active then
		local background2info = C.GetDropDownBackground2Details(dropdownID)
		widgetSystem.setElementScale(background2element, (background2info.width - 2 * private.scaledSizes.dropdown_border) / 100, (background2info.height - 2 * private.scaledSizes.dropdown_border) / 100)
		widgetSystem.setElementPosition(background2element, background2info.x, background2info.y, (width % 2 ~= 0), (height % 2 ~= 0))
		dropdownElement.background2color = {
			["r"] = background2info.color.red,
			["g"] = background2info.color.green,
			["b"] = background2info.color.blue,
			["a"] = background2info.color.alpha,
			["glow"] = background2info.color.glow,
		}
	else
		dropdownElement.background2color = Color["dropdown_background_hidden"]
	end
	SetDiffuseColor(background2material, dropdownElement.background2color.r, dropdownElement.background2color.g, dropdownElement.background2color.b)
	setAttribute(background2material, "opacity", dropdownElement.background2color.a)
	setAttribute(background2material, "glowfactor", dropdownElement.background2color.glow)
	...
	for _, element in ipairs(config.dropdown.borderElements) do
		...
		widgetSystem.setElementScale(anarkElement, private.scaledSizes.dropdown_border / 100, private.scaledSizes.dropdown_border / 100)
	...
	for i = 0, n - 1 do
		...
		for j = 1, config.dropdown.numtexts do
			entry["text" .. j] = ffi.string(buf[i]["text" .. j])
		end
	...
	local buf = ffi.new("UIOverlayInfo3[?]", n)
	n = C.GetDropDownOptionOverlayInfo2(buf, n, dropdownID)
	for i = 0, n - 1 do
	...
	widgetSystem.setElementPosition(arrowElement, (dropdownElement.rightSideArrow and 1 or -1) * (width / 2 - config.texturesizes.dropdown.borderSize - dropdownElement.arrowHeight / 2))
	...
	-- textDetails
	for i = 1, config.dropdown.numtexts do
		local key = "text" .. i .. "Details"

		dropdownElement[key] = {}
		local buf = ffi.new("DropDownTextInfo")
		if C.GetDropDownTextProperties(dropdownID, i, buf) then
			dropdownElement[key].color = {}
			dropdownElement[key].color.red = buf.color.red
			dropdownElement[key].color.green = buf.color.green
			dropdownElement[key].color.blue = buf.color.blue
			dropdownElement[key].color.alpha = buf.color.alpha
			dropdownElement[key].fontname = ffi.string(buf.font.name)
			dropdownElement[key].fontsize = buf.font.size
			dropdownElement[key].alignment = ffi.string(buf.alignment)
			dropdownElement[key].x = buf.x
			dropdownElement[key].y = buf.y
			dropdownElement[key].textOverride = ffi.string(buf.textOverride)
			dropdownElement[key].glowfactor = buf.glowfactor

			dropdownElement["text" .. i .. "Active"] = true
		end

		if dropdownElement["text" .. i .. "Active"] then
			-- calculate horizontal offset for proper alignment
			if dropdownElement[key].alignment == "left" then
				dropdownElement[key].x = dropdownElement[key].x - width/2 + config.texturesizes.button.borderSize + (dropdownElement.rightSideArrow and 0 or dropdownElement.arrowHeight)
			elseif dropdownElement[key].alignment == "center" then
				dropdownElement[key].x = 0
			else -- right
				dropdownElement[key].x = -dropdownElement[key].x + width/2 - config.texturesizes.button.borderSize - (dropdownElement.rightSideArrow and dropdownElement.arrowHeight or 0)
			end
			local textelement = getElement("Text" .. i, dropdownElement.element.mainElement)
			widgetSystem.setElementPosition(textelement, dropdownElement[key].x, dropdownElement[key].y)
			setAttribute(textelement, "horzalign", widgetSystem.convertAlignment(dropdownElement[key].alignment))
			-- text font
			setAttribute(textelement, "font", dropdownElement[key].fontname)
			setAttribute(textelement, "size", dropdownElement[key].fontsize)
	...
	if dropdownElement.iconActive then
		...
		if dropdownElement.iconDetails.width ~= 0 and dropdownElement.iconDetails.height ~= 0 then
			-- icon offsets only apply, if we do not have a full-sized icon (aka: iconWidth and/or iconHeight is != 0)
			dropdownElement.iconDetails.x = dropdownElement.iconDetails.x - width/2 + dropdownElement.iconDetails.width / 2 + (dropdownElement.rightSideArrow and 0 or dropdownElement.arrowHeight)
			dropdownElement.iconDetails.y = - dropdownElement.iconDetails.y + height/2 - dropdownElement.iconDetails.height / 2
		else
			dropdownElement.iconDetails.x = (dropdownElement.rightSideArrow and -1 or 1) * dropdownElement.arrowHeight / 2
...
function widgetSystem.setUpEditBox(frameElement, editboxID, editboxElement, parentx, parenty, parentwidth, parentheight, backgroundcolspanwidth)
	...
	editboxElement.widgetActive = widgetSystem.isWidgetActive(editboxID) and C.IsEditBoxActive(editboxID)
	...
	widgetSystem.setElementScale(getElement("background_black", editboxElement.element), (width - 2 * private.scaledSizes.editbox_border) / 100, (height - 2 * private.scaledSizes.editbox_border) / 100)
...
function widgetSystem.setUpFlowchart(frameElement, flowchartID, flowchartElement)
	...
	flowchartData.contentOffsety = flowchartData.offsety + flowchartData.borderHeight + private.scaledSizes.table_borderSize	-- add top border and inner padding
	flowchartData.maxVisibleHeight = flowchartData.height - 2 * (flowchartData.borderHeight + private.scaledSizes.table_borderSize)	-- subtract borders and inner padding
	...
	if flowchartData.fullContentHeight > flowchartData.maxVisibleHeight - flowchartData.maxCaptionHeight then
		...
		flowchartData.maxVisibleWidth = flowchartData.maxVisibleWidth - private.scaledSizes.scrollBar_width - private.scaledSizes.scrollBar_borderSize
	end
	if flowchartData.fullContentWidth > flowchartData.maxVisibleWidth then
		...
		flowchartData.maxVisibleHeight = flowchartData.maxVisibleHeight - private.scaledSizes.scrollBar_width - private.scaledSizes.scrollBar_borderSize
	...
	if flowchartData.borderHeight > 0 then
		...
		local bordery_bottom = parenty - (flowchartData.contentOffsety + flowchartData.maxVisibleHeight + private.scaledSizes.table_borderSize + flowchartData.borderHeight / 2)
	...
	if flowchartData.hasVerticalScrollBar then
		local scrollbarx = parentx + (flowchartData.offsetx + flowchartData.width - private.scaledSizes.scrollBar_width)
	...
	if flowchartData.hasHorizontalScrollBar then
		...
		local scrollbary = parenty - (flowchartData.offsety + flowchartData.height - private.scaledSizes.scrollBar_width)
	...
	if frameElement.frameborders[flowchartData.frameBorderID] then
		widgetSystem.updateFrameBorderExtents(frameElement.frameborders[flowchartData.frameBorderID], parentx + flowchartData.offsetx, parenty - flowchartData.offsety, flowchartData.width, flowchartData.height)
	end
...
function widgetSystem.setUpBoxText(boxtextID, boxtextElement, parentx, parenty, parentwidth, parentHeight, parentBGWidth, isPrevColBGSpanned, isColBGSpanned)
	...
	if isPrevColBGSpanned then
		...
		borderWidth = borderWidth + private.scaledSizes.table_borderSize
		borderOffset = borderOffset - private.scaledSizes.table_borderSize / 2
	end
...
function widgetSystem.setUpGraph(graphID, graphElement, parentx, parenty, parentwidth, parentheight, frameElement, backgroundcolspanwidth)
	...
	widgetSystem.setUpGraphAxis(graphID, xAxisElement, C.GetGraphXAxis2, graphElement.xAxis)

	-- yAxis (1/2)
	graphElement.yAxis = {}
	local yAxisElement = getElement("axis2", graphElement.element.mainElement)
	widgetSystem.setUpGraphAxis(graphID, yAxisElement, C.GetGraphYAxis2, graphElement.yAxis)
	...
	-- optional -> select initial datapoint
	local selectionInfo = ffi.new("GraphInitialSelectionInfo")
	selectionInfo = C.GetGraphInitialSelectionInfo(graphID)
	local recordIdx = tonumber(selectionInfo.InitialSelectedRecordIdx)
	local dataIdx = tonumber(selectionInfo.InitialSelectedDataIdx)
	if (recordIdx > 0) and (dataIdx > 0) then
		SelectGraphDataPoint(graphID, recordIdx, dataIdx, true)
		-- print("InitialGraphDataPoint: " .. recordIdx .. "," .. dataIdx)
	end
end
...
function widgetSystem.setUpGraphAxis(graphID, axisElement, accessor, axisData)
	...
	local axis = ffi.new("GraphAxisInfo2")
	if accessor(graphID, axis) then
		-- axis unittext
		axisData.unittext = ffi.string(axis.unittext)
		-- axis label
		axisData.label = {}
		...
		if axisData.label.text ~= "" then
			...
			-- label text
			if axisData.unittext ~= "" then
				setAttribute(axisLabelElement, "textstring", axisData.label.text .. " [" .. axisData.unittext .. "]" ) -- append unittext
			else
...
function widgetSystem.setUpRenderTarget(frameElement, renderTargetID, renderTargetElement)
	...
	local frameborderid = ffi.string(C.GetRenderTargetFrameBorderID(renderTargetID))
	if frameborderid and frameElement.frameborders[frameborderid] then
		widgetSystem.updateFrameBorderExtents(frameElement.frameborders[frameborderid], globalx, globaly, width, height)
	end
...
function widgetSystem.setUpSliderCell(frameElement, slidercellID, slidercellElement, parentx, parenty, parentwidth, parentheight, backgroundcolspanwidth)
	...
	if C.GetSliderCellValues(slidercellID, values) then
		...
		slidercellElement.readonly = (not widgetSystem.isWidgetActive(slidercellID)) or values.readonly
	...
	if not slidercellElement.readonly then
		bgwidth = bgwidth - 2 * private.scaledSizes.slidercell_border
		bgheight = bgheight - 2 * private.scaledSizes.slidercell_border
	end
	widgetSystem.setElementScale(getElement("slider.elements.Rectangle_background", slidercellElement.element.main), bgwidth / 100, bgheight / 100)
	widgetSystem.setElementScale(getElement("slider.elements.border", slidercellElement.element.main), width / 100, height / 100)
	...
	-- border color
	local bordercolor = ffi.new("GlowColor")
	if C.GetSlidercellBorderColor(slidercellID, bordercolor) then
		local material = getElement("slider.elements.border.Material651", slidercellElement.element.main)
		SetDiffuseColor(material, bordercolor.red, bordercolor.green, bordercolor.blue)
		setAttribute(material, "opacity", bordercolor.alpha)
		setAttribute(material, "glowfactor", bordercolor.glow)
	end
...
function widgetSystem.setUpTable(frameElement, tableID, tableElement)
	...
	local _, tableheight = GetSize(tableID)
	local maxtableheight = tableheight
	...
	if (tableheight > 0) and (maxtableheight <= 0) then
	...
	tableElement.borderbelowrows = {}
	tableElement.paddingrows = {}
	tableElement.rowgrouprows = {}

	local buf = ffi.new("TableRowInfo[?]", numRows)
	local num_rowinfo = C.GetTableRowInfo(buf, numRows, tableID)
	for i = 0, num_rowinfo - 1 do
		local row = i + 1
		-- set unselectable rows
		if not buf[i].selectable then
			tableElement["unselectableRows"][row] = true
		end
		-- set noninteratctive rows
		if not buf[i].interactive then
			tableElement["noninteractiveRows"][row] = true
		end
		-- set rows with no borderbelow
		if row < numRows then
			if buf[i].borderbelow then
				tableElement["borderbelowrows"][row] = true
				numBorderRows = numBorderRows + 1
			end
		end
		-- set rows with rowpadding
		tableElement["paddingrows"][row] = { top = buf[i].padding.top, bottom = buf[i].padding.bottom }
		-- get rowgroup if any
		if buf[i].group > 0 then
			tableElement["rowgrouprows"][row] = buf[i].group
		end
	end
	...
	tableElement["rowGroups"] = {}
	tableElement["rowGroupsByFirstRow"] = {}
	local numrowgroups = C.GetNumTableRowGroups(tableID)
	if numrowgroups > 0 then
		local buf = ffi.new("RowGroupInfo[?]", numrowgroups)
		n =	C.GetTableRowGroups(buf, numrowgroups, tableID)
		for i = 0, numrowgroups - 1 do
			local entry = {
				groupid = buf[i].groupid,
				level = buf[i].level,
				firstrow = buf[i].firstrow,
				numrows = buf[i].numrows,
				color = { r = buf[i].color.red, g = buf[i].color.green, b = buf[i].color.blue, a = buf[i].color.alpha, glow = buf[i].color.glow },
			}
			tableElement.rowGroups[entry.groupid] = entry
			tableElement.rowGroupsByFirstRow[entry.firstrow] = entry
		end
	end

	-- check whether or not the table should display a scrollbar
	local fullrowheight          = GetTableFullHeight(tableID)
	local requiredRowHeight      = fullrowheight + numBorderRows * private.scaledSizes.table_borderSize + 2 * numrowgroups * private.scaledSizes.tableRowGroups_borderSize
	-- #StefanLow - isn't requiredTableHeight a more suitable name
	requiredRowHeight            = math.max(requiredRowHeight, 0) -- empty tables result in a calculated height of -private.scaledSizes.table_borderSize, hence set that to 0
	...
	if scrollBar and (numRows > numTableRowElements) and (maxtableheight >= requiredRowHeight) and (not columnsInPercent) then
		...
		if not haszero and maxWidthIdx then
			if maxWidth > private.scaledSizes.scrollBar_width + private.scaledSizes.scrollBar_borderSize then
				columnWidths[maxWidthIdx] = columnWidths[maxWidthIdx] - private.scaledSizes.scrollBar_width - private.scaledSizes.scrollBar_borderSize
			end
		end
	end
	...
	if scrollBar then
		...
		maxtablewidth = maxtablewidth - private.scaledSizes.scrollBar_width - private.scaledSizes.scrollBar_borderSize
	end
	...
	local maxtablewidthexcludingborders = maxtablewidth - (numCols - 1) * private.scaledSizes.table_borderSize
	local success = widgetSystem.convertColumnWidth(columnWidths, columnsInPercent, maxtablewidthexcludingborders)
	-- #StefanMed XT-2500
	local tablewidth = widgetSystem.summarize(columnWidths) + (numCols - 1) * private.scaledSizes.table_borderSize
	if not success then
	...
	if scrollBar then
		fixedRowColumnWidths[#fixedRowColumnWidths] = fixedRowColumnWidths[#fixedRowColumnWidths] + private.scaledSizes.scrollBar_width + private.scaledSizes.scrollBar_borderSize
	end
	local fixedrowwidth = widgetSystem.summarize(fixedRowColumnWidths) + (numCols - 1) * private.scaledSizes.table_borderSize
	...
	tableElement.fixedrowwidth = fixedrowwidth
	...
	for col = 2, numCols do
		table.insert(cellposx, cellposx[col - 1] + columnWidths[col - 1] + private.scaledSizes.table_borderSize)
	end
	...
	for col = 2, numCols do
		table.insert(fixedRowCellposx, fixedRowCellposx[col - 1] + fixedRowColumnWidths[col - 1] + private.scaledSizes.table_borderSize)
	end
	...
	tableElement.frameBorderID = ffi.string(C.GetTableFrameBorderID(tableID))
	if not widgetSystem.isPanelActive(tableElement.frameBorderID) then
		tableElement.highlightMode = "off"
	end
	...
	tableElement.height = math.min(tableElement.height, math.max(curtableheight, mintableheight))
	...
	if scrollBar then
		widgetSystem.setUpVerticalScrollBar(tableElement.scrollBar, globalx + tablewidth + private.scaledSizes.scrollBar_borderSize, globaly - fixedRowHeight, tableElement.nonFixedSectionHeight, tableElement.nonFixedSectionHeight, requiredRowHeight, not widgetSystem.isPanelActive(tableElement.frameBorderID))
	end
	...
	local mousepickwidth = tablewidth + (scrollBar and (private.scaledSizes.scrollBar_width + private.scaledSizes.scrollBar_borderSize) or 0)
	...
	if tableElement.frameBorderID and frameElement.frameborders[tableElement.frameBorderID] then
		widgetSystem.updateFrameBorderExtents(frameElement.frameborders[tableElement.frameBorderID], globalx, globaly, mousepickwidth, tableElement.height)
	end
...
function widgetSystem.setUpVerticalScrollBar(scrollBar, posx, posy, scrollbarHeight, visibleParentHeight, fullParentHeight, readonly)
	...
	widgetSystem.setElementPosition(scrollBarElement, posx + private.scaledSizes.scrollBar_width / 2, posy - scrollbarHeight / 2)
	...
	widgetSystem.setElementScale(background, private.scaledSizes.scrollBar_width / 100, scalefactor)

	local color = readonly and Color["scrollbar_background_inactive"] or Color["scrollbar_background"]
	...
	local mousepickElement = getElement("mousepick", centerSliderElement)
	...
	widgetSystem.setElementScale(centerSliderElement, private.scaledSizes.scrollBar_sliderWidth / 12, scalefactor)
	local mousepickwidth = (2 * private.scaledSizes.scrollBar_borderSize + private.scaledSizes.scrollBar_sliderWidth) / private.scaledSizes.scrollBar_sliderWidth
	widgetSystem.setElementScale(mousepickElement, mousepickwidth / 12, getAttribute(mousepickElement, "scale.y"))

	local color = readonly and Color["scrollbar_slider_inactive"] or Color["scrollbar_slider_normal"]
	local material = getElement("center.Material600", centerSliderElement)
	SetDiffuseColor(material, color.r, color.g, color.b)
	setAttribute(material, "opacity", color.a)
	setAttribute(material, "glowfactor", color.glow)

	local color = readonly and Color["scrollbar_slider_inactive"] or Color["scrollbar_slider_highlight"]
	local material = getElement("center_highlight.Material1144", centerSliderElement)
	SetDiffuseColor(material, color.r, color.g, color.b)
	setAttribute(material, "opacity", color.a)
	setAttribute(material, "glowfactor", color.glow)

	local color = readonly and Color["scrollbar_slider_inactive"] or Color["scrollbar_slider_click"]
...
function widgetSystem.setUpHorizontalScrollBar(scrollBar, posx, posy, scrollbarWidth, visibleParentWidth, fullParentWidth)
	...
	widgetSystem.setElementPosition(scrollBarElement, posx + scrollbarWidth / 2, posy - private.scaledSizes.scrollBar_width / 2)

	-- set background element
	local background  = getElement("background.center", scrollBarElement)
	local scalefactor = scrollbarWidth / 100
	widgetSystem.setElementScale(background, scalefactor, private.scaledSizes.scrollBar_width / 100)
	...
	widgetSystem.setElementScale(centerSliderElement, scalefactor, private.scaledSizes.scrollBar_sliderWidth / 12)
...
function widgetSystem.updateSliderCellValue(slidercellElement)
	...
	if not slidercellElement.readonly then
		width = width - 2 * private.scaledSizes.slidercell_border
		height = height - 2 * private.scaledSizes.slidercell_border
	end
	widgetSystem.setElementPosition(background2, (xstart + xend) / 2 * width, 0, (slidercellElement.width % 2 ~= 0), 0)
	widgetSystem.setElementScale(background2, (xend - xstart) * width / 100, height / 100)
end
...
function widgetSystem.unsetInteractiveChildElement(tableElement, childWidgetID, childTableElement)
	...
	if private.enableMouseOverText then
		if private.mouseOverText and (not private.gamepadTableMouseOver) and (private.mouseOverText.posOverride ~= nil) then
...
function widgetSystem.updateButton(buttonID, buttonElement)
	...
	buttonElement.bordercolor = buttonElement.active and Color["button_border_default"] or Color["button_border_inactive"]
	if buttonElement.active then
		local color = ffi.new("GlowColor")
		if C.GetButtonBorderColor(buttonID, color) then
			buttonElement.bordercolor = {
				["r"] = color.red,
				["g"] = color.green,
				["b"] = color.blue,
				["a"] = color.alpha,
...
function widgetSystem.updateButtonColor(buttonID, buttonElement, colorMode)
	...
	local material = getElement("border.mat_value", buttonElement.element)
	SetDiffuseColor(material, buttonElement.bordercolor.r, buttonElement.bordercolor.g, buttonElement.bordercolor.b)
	setAttribute(material, "opacity", buttonElement.bordercolor.a)
	setAttribute(material, "glowfactor", buttonElement.bordercolor.glow or 0)

	if not private.fadingButtonHighlights[buttonID] then
		local color = buttonElement.color
		if colorMode == "click" then
			color = Color["button_background_inactive"]
		elseif colorMode == "highlight" then
			color = buttonElement.highlightcolor
		end
		local material = getElement("background.Material753", buttonElement.element)
		SetDiffuseColor(material, color.r, color.g, color.b)
		setAttribute(material, "opacity", color.a)
	...
	if buttonElement.active then
		...
		elseif stateEntry.mouseOver or stateEntry.keyboard then
			...
			if stateEntry.highlight then
				colorMode = "highlight"
				private.fadingButtonHighlights[buttonID] = getElapsedTime()
			end
...
function widgetSystem.updateCheckBoxColor(checkbox, checkboxElement, colorMode)
	...
	local material = getElement("background_black.Material1365", checkboxElement.element)
	SetDiffuseColor(material, color.r, color.g, color.b)
	setAttribute(material, "opacity", color.a)
	-- symbol
	local material
...
function widgetSystem.updateDropDown(dropdownID, dropdownElement)
	for i = 1, config.dropdown.numtexts do
		...
		local arrowwidth
		if dropdownElement[key].alignment == "center" then
		...
		local text = ""
		if dropdownElement["text" .. i .. "Active"] then
			if dropdownElement[key].textOverride ~= "" then
				text = dropdownElement[key].textOverride
			else
				if dropdownElement.curOption.deleted then
					text = "---"
				else
					text = dropdownElement.curOption["text" .. i] or ((i == 1) and "---" or "")
				end
			end
		...
		text = TruncateText(text, dropdownElement[key].fontname, dropdownElement[key].fontsize, dropdownElement.width - 2 * config.dropdown.optionSpacing - arrowwidth)
		setAttribute(textelement, "textstring", text)
	end
	...
	if C.GetDropDownHighlightColor(dropdownID, color) then
		...
		dropdownElement.highlightcolor = Color["dropdown_highlight_default"]
	end
end
...
function widgetSystem.updateDropDownColor(dropdownID, dropdownElement, colorMode)
	...
	for i = 1, config.dropdown.numtexts do
		...
		if dropdownElement["text" .. i .. "Active"] then
			local r, g, b, a = dropdownElement[key].color.red, dropdownElement[key].color.green, dropdownElement[key].color.blue, dropdownElement[key].color.alpha
			if colorMode == "inverse" then
				r, g, b = widgetSystem.invertColor(r, g, b)
			elseif colorMode == "gray" then
				local color = Color["dropdown_text_inactive"]
			...
			local text2element = getElement("Text" .. i, dropdownElement.element.mainElement)
			...
			setAttribute(text2element, "glowfactor", dropdownElement[key].glowfactor)
		end
	end

	-- use first text color for arrow
	local r, g, b, a = dropdownElement.text1Details.color.red, dropdownElement.text1Details.color.green, dropdownElement.text1Details.color.blue, dropdownElement.text1Details.color.alpha
	if colorMode == "inverse" then
		r, g, b = widgetSystem.invertColor(r, g, b)
	elseif colorMode == "gray" then
	...
	if IsValidWidgetElement(dropdownID) then
		if dropdownElement.active then
			...
			if colorMode == "click" then
				color = Color["dropdown_background_inactive"]
			elseif colorMode == "highlight" then
		...
		local color = dropdownElement.bordercolor
		if colorMode == "highlight" then
			color = dropdownElement.highlightcolor
		end
		local material = getElement("border_main.mat_value", dropdownElement.element.mainElement)
		SetDiffuseColor(material, color.r, color.g, color.b)
		setAttribute(material, "opacity", color.a)
		setAttribute(material, "glowfactor", color.glow or 0)
...
function widgetSystem.truncateDropDownOptionText(dropdownElement, option, index)
	local key = "text" .. index .. "Details"
	...
	return TruncateText(option["text" .. index], (option.font ~= "") and option.font or dropdownElement[key].fontname, dropdownElement[key].fontsize, dropdownElement.actualWidth - 2 * config.dropdown.optionSpacing - arrowwidth - removeoptionwidth)
end
...
function widgetSystem.updateDropDownOptions(frameElement, dropdownID, dropdownElement, topOption, force)
	...
	for i = dropdownElement.topOption, dropdownElement.noofVisibleOptions + dropdownElement.topOption - 1 do
		...
		local optionElementIndex = i - dropdownElement.topOption + 1
		local optionElement = dropdownElement.element.optionElements[optionElementIndex]
		...
		if option.active then
			...
			if optionElementIndex == private.activeDropDown.highlighted then
				color = dropdownElement.highlightcolor
				highlighedOptionID = option.id
			end
			SetDiffuseColor(material, color.r, color.g, color.b)
			setAttribute(material, "opacity", color.a)
			setAttribute(material, "glowfactor", color.glow)
		...
		for i = 1, config.dropdown.numtexts do
			...
			local textelement = getElement("Text" .. i, optionElement)
			if dropdownElement["text" .. i .. "Active"] then
				local text = widgetSystem.truncateDropDownOptionText(dropdownElement, option, i)
				option["truncatedtext" .. i] = text

				local color = dropdownElement[key].color
				...
				setAttribute(textelement, "glowfactor", dropdownElement[key].glowfactor)
		...
		if dropdownElement.iconActive then
			...
			if option.icon ~= "" then
				widgetSystem.setElementPosition(iconElement, dropdownElement.iconDetails.x - (dropdownElement.scrollBar and (private.scaledSizes.scrollBar_width / 2) or 0), dropdownElement.iconDetails.y)
				goToSlide(iconElement, "active")
		...
		local y = dropdownElement.offsety + (dropdownElement.flipped and (dropdownElement.noofVisibleOptions - (i - dropdownElement.topOption)) or -optionElementIndex) * (dropdownElement.height + config.dropdown.optionSpacing)
	...
	CallWidgetEventScripts(dropdownID, "onDropDownOptionChanged", highlighedOptionID)
end
...
function widgetSystem.showDropDownOptions(frameElement, dropdownID, dropdownElement)
	...
	if actualmaxheight < dropdownElement.fullHeight then
		...
		actualwidth = actualwidth - private.scaledSizes.scrollBar_width - private.scaledSizes.scrollBar_borderSize
	end
	...
	-- border elements
	local height = dropdownElement.fullHeight - dropdownElement.height + private.scaledSizes.dropdown_border
	local sideOffsetY = (dropdownElement.flipped and 1 or -1) * (dropdownElement.noofVisibleOptions +   1) * (dropdownElement.height + config.dropdown.optionSpacing) / 2 - (private.scaledSizes.dropdown_border - config.dropdown.optionSpacing) / 2
	local downOffsetY = (dropdownElement.flipped and 1 or -1) * (dropdownElement.noofVisibleOptions + 0.5) * (dropdownElement.height + config.dropdown.optionSpacing)     - (private.scaledSizes.dropdown_border - config.dropdown.optionSpacing) / 2

	-- left
	local leftBorderElement = getElement("border_left", dropdownElement.element.mainElement)
	widgetSystem.setElementPosition(leftBorderElement, -(dropdownElement.width - private.scaledSizes.dropdown_border) / 2, sideOffsetY, (dropdownElement.width % 2 ~= 0), (dropdownElement.height % 2 ~= 0))
	widgetSystem.setElementScale(leftBorderElement, private.scaledSizes.dropdown_border / 100, height / 100)
	...
	widgetSystem.setElementPosition(rightBorderElement, (dropdownElement.width - private.scaledSizes.dropdown_border) / 2 + (dropdownElement.optionwidth - dropdownElement.width), sideOffsetY, (dropdownElement.width % 2 ~= 0), (dropdownElement.height % 2 ~= 0))
	widgetSystem.setElementScale(rightBorderElement, private.scaledSizes.dropdown_border / 100, height / 100)
	...
	widgetSystem.setElementScale(downBorderElement, (math.max(dropdownElement.width, dropdownElement.optionwidth) - 2 * private.scaledSizes.dropdown_border) / 100, private.scaledSizes.dropdown_border / 100)
	...
	if dropdownElement.scrollBar then
		...
		widgetSystem.setUpVerticalScrollBar(dropdownElement.element.scrollBar, dropdownElement.optionwidth + (-dropdownElement.width - private.scaledSizes.scrollBar_borderSize) / 2 - private.scaledSizes.scrollBar_width - private.scaledSizes.dropdown_border, -dropdownElement.height / 2 - config.dropdown.optionSpacing, visibleOptionsHeight, visibleOptionsHeight, fullOptionsHeight)
	...
	for i = dropdownElement.topOption, dropdownElement.noofVisibleOptions + dropdownElement.topOption - 1 do
		...
		for i = 1, config.dropdown.numtexts do
			...
			if dropdownElement["text" .. i .. "Active"] then
				-- calculate horizontal offset for proper alignment
				local textXOffset = dropdownElement[key].x
				local textYOffset = dropdownElement[key].y
				if dropdownElement[key].alignment == "left" then
	...
	widgetSystem.updateDropDownOptionState(dropdownID, dropdownElement, 0)
...
function widgetSystem.hideDropDownOptions(dropdownID, dropdownElement)
	...
	CallWidgetEventScripts(dropdownID, "onDropDownDeactivated")
end
...
function widgetSystem.updateDropDownOption(dropdownElement)
	...
	widgetSystem.updateDropDownOptionState(dropdownID, dropdownElement, oldhighlighted)
...
function widgetSystem.updateDropDownOptionState(dropdownID, dropdownElement, oldhighlighted)
	if private.activeDropDown.highlighted ~= oldhighlighted then
		if private.activeDropDown.highlighted ~= 0 then
			...
			CallWidgetEventScripts(dropdownID, "onDropDownOptionChanged", dropdownElement.options[highlightedoption].id)
			if dropdownElement.options[highlightedoption].active then
				local material = getElement("background.Material753", dropdownElement.element.optionElements[private.activeDropDown.highlighted])
				local color = dropdownElement.highlightcolor
				SetDiffuseColor(material, color.r, color.g, color.b)
				setAttribute(material, "opacity", color.a)
				setAttribute(material, "glowfactor", color.glow)
			end
		...
		if oldhighlighted ~= 0 then
			...
			if dropdownElement.options[oldhighlightedoption].active then
				local material = getElement("background.Material753", dropdownElement.element.optionElements[oldhighlighted])
				local color = Color["dropdown_option_background"]
				SetDiffuseColor(material, color.r, color.g, color.b)
				setAttribute(material, "opacity", color.a)
				setAttribute(material, "glowfactor", color.glow)
...
function widgetSystem.updateFrame(frameID)
	...
	frameElement.frameborders = {}
	local n = C.GetNumFrameBorders(frameID)
	if n > 0 then
		local buf = ffi.new("UIFrameBorderInfo[?]", n)
		n = C.GetFrameBorders(buf, n, frameID)
		for i = 0, n - 1 do
			local id = ffi.string(buf[i].id)
			local active = buf[i].active
			frameElement.frameborders[id] = {
				linewidth = buf[i].linewidth,
				offsets = { top = buf[i].offsetTop, bottom = buf[i].offsetBottom, left = buf[i].offsetLeft, right = buf[i].offsetRight },
				color = { r = buf[i].color.red, g = buf[i].color.green, b = buf[i].color.blue, a = buf[i].color.alpha, glow = buf[i].color.glow },
				active = active,
				icon = {
					iconid = ffi.string(buf[i].icon.iconid),
					swapiconid = ffi.string(buf[i].icon.swapiconid),
					anchorid = ffi.string(buf[i].icon.anchorid),
					color = { r = buf[i].icon.color.red, g = buf[i].icon.color.green, b = buf[i].icon.color.blue, a = buf[i].icon.color.alpha, glow = buf[i].icon.color.glow },
					width = buf[i].icon.width,
					height = buf[i].icon.height,
					x = buf[i].icon.x,
					y = buf[i].icon.y,
				},
				icon2 = {
					iconid = ffi.string(buf[i].icon2.iconid),
					swapiconid = ffi.string(buf[i].icon2.swapiconid),
					anchorid = ffi.string(buf[i].icon2.anchorid),
					color = { r = buf[i].icon2.color.red, g = buf[i].icon2.color.green, b = buf[i].icon2.color.blue, a = buf[i].icon2.color.alpha, glow = buf[i].icon2.color.glow },
					width = buf[i].icon2.width,
					height = buf[i].icon2.height,
					x = buf[i].icon2.x,
					y = buf[i].icon2.y,
				},
			}
			private.activeFrameBorders[id] = active
		end
	end
	...
	if not private.miniWidgetSystemUsed then
		for id, frameborder in pairs(frameElement.frameborders) do
			if frameborder.x ~= nil then
				local frameBorderElement = widgetSystem.getElement("frameborder")
				if frameBorderElement then
					frameElement.frameborders[id].element = frameBorderElement
					widgetSystem.showFrameElementBorder(frameBorderElement, frameborder)
					widgetSystem.moveAnarkElementFrameLayer(frameElement, frameBorderElement, 1)
				end
			end
		end
	end
...
function widgetSystem.updateFrameBorderExtents(frameborder, x, y, width, height)
	if frameborder.x == nil then
		frameborder.x = x
		frameborder.y = y
		...
		if frameborder.x > x then
			local diff = frameborder.x - x
			frameborder.x = x
			frameborder.width = frameborder.width + diff
		end
		if frameborder.y < y then
			local diff = y - frameborder.y
			frameborder.y = y
			frameborder.height = frameborder.height + diff
		end
		if frameborder.x + frameborder.width < x + width then
			frameborder.width = x - frameborder.x + width
		end
		if frameborder.y - frameborder.height > y - height then
			frameborder.height = frameborder.y - y + height
		end
	end
end

function widgetSystem.isWidgetActive(widgetID)
	return widgetSystem.isPanelActive(private.panelAssociation[widgetID])
end

function widgetSystem.isPanelActive(frameBorderID)
	if not private.panelMode then
		return true
	end
	if (frameBorderID == nil) or (frameBorderID == "") then
		return true
	end
	return private.activeFrameBorders[frameBorderID]
end
...
function widgetSystem.deactivateEditBox(editboxID, editboxElement, isconfirmed, wastableclick)
	...
	if editboxID and IsValidWidgetElement(editboxID) then
...
function widgetSystem.getWidgetSystemSizes()
	return {
		width = private.width,
		height = private.height,
		table_borderSize = private.scaledSizes.table_borderSize,
		tableRowGroups_borderSize = private.scaledSizes.tableRowGroups_borderSize,
		scrollBar_width = private.scaledSizes.scrollBar_width,
		scrollBar_sliderWidth = private.scaledSizes.scrollBar_sliderWidth,
		scrollBar_borderSize = private.scaledSizes.scrollBar_borderSize,
	}
end

function widgetSystem.createInputBar(height, inputsLeft, inputsRight)
	local uiscale = C.GetUIScale(true)
	-- limit the user UI scale for the inputbar and limit it to 1.4 to help with overlap issues. Re-apply screen size scaling.
	uiscale = math.min(1.4, uiscale) * private.uiscale / uiscale

	private.inputBar = {
		height = math.floor(height * uiscale + 0.5),
		inputsLeft = inputsLeft or {},
		inputsRight = inputsRight or {},
	}
	private.height = private.height - private.inputBar.height

	-- elements
	local inputBarElement = getElement("inputBar", private.widgetsystem)
	local bgElement = getElement("background", inputBarElement)
	local borderElement = getElement("border", inputBarElement)
	local textLeftElement = getElement("TextLeft", inputBarElement)
	local textRightElement = getElement("TextRight", inputBarElement)

	-- background
	widgetSystem.setElementPosition(bgElement, 0, -private.height / 2, false, (private.height / 2) % 2 ~= 0)
	widgetSystem.setElementScale(bgElement, private.width / 100, private.inputBar.height / 100)

	local color = Color["inputbar_background"]
	local bgMaterial = getElement("Material1361", bgElement)
	widgetSystem.setElementColorAlpha(bgMaterial, color)

	-- border
	local borderSize = math.floor(config.inputBar.borderSize * uiscale + 0.5)
	widgetSystem.setElementPosition(borderElement, 0, -(private.height - private.inputBar.height - borderSize) / 2, false, (private.height / 2) % 2 ~= 0)
	widgetSystem.setElementScale(borderElement, private.width / 100, borderSize / 100)

	local color = Color["inputbar_border"]
	local borderMaterial = getElement("Material1367", borderElement)
	widgetSystem.setElementColorAlpha(borderMaterial, color)

	-- text
	local textoffset = math.floor(config.inputBar.textOffset * uiscale + 0.5)
	local textfont = getAttribute(textLeftElement, "font")
	local textsize = math.ceil(config.inputBar.fontSize * uiscale)
	local textheight = C.GetTextHeight("MMM", textfont, textsize, 0)
	local textOffsetX = -private.width / 2 + textoffset
	local textOffsetY = -(private.height - private.inputBar.height + textheight - textoffset / 2) / 2

	widgetSystem.setElementPosition(textLeftElement, textOffsetX, textOffsetY, (private.width / 2) % 2 ~= 0, (private.height / 2) % 2 ~= 0)
	setAttribute(textLeftElement, "size", textsize)
	widgetSystem.setElementTextColor(textLeftElement, Color["text_normal"])

	widgetSystem.setElementPosition(textRightElement, -textOffsetX, textOffsetY, (private.width / 2) % 2 ~= 0, (private.height / 2) % 2 ~= 0)
	setAttribute(textRightElement, "size", textsize)
	widgetSystem.setElementTextColor(textRightElement, Color["text_normal"])
end

function widgetSystem.updateInputBar(inputsLeft, inputsRight)
	if not private.inputBar then
		DebugError("Widget system error. Input Bar is not active and cannot be updated. Call CreateInputBar() first.")
		return
	end

	private.inputBar.inputsLeft = inputsLeft or {}
	private.inputBar.inputsRight = inputsRight or {}

	local inputBarElement = getElement("inputBar", private.widgetsystem)
	local textLeftElement = getElement("TextLeft", inputBarElement)
	local textRightElement = getElement("TextRight", inputBarElement)

	setAttribute(textLeftElement,  "textstring", widgetSystem.getInputBarText(private.inputBar.inputsLeft))
	setAttribute(textRightElement, "textstring", widgetSystem.getInputBarText(private.inputBar.inputsRight))
end

function widgetSystem.getInputBarText(inputs)
	local text = ""
	for i, entry in ipairs(inputs) do
		if entry.linebreak then
			text = text .. "\n"
		else
			if i > 1 then
				text = text .. "    "
			...
			if entry.modifiers then
				for j, modifier in ipairs(entry.modifiers) do
					if j > 1 then
						text = text .. "+"
					end
					text = text .. ffi.string(C.GetDisplayedModifierKey(modifier))
				end
				if entry.inputs or entry.icons then
					text = text .. "+"
				end
			end
			if entry.inputs then
				for j, input in ipairs(entry.inputs) do
					if j > 1 then
						text = text .. " "
					end
					local inputname = ffi.string(C.GetMappedInputName(input))
					if utf8.sub(inputname, 1, 2) ~= "\27[" then
						inputname = ColorText["input_reference"] .. inputname .. "\27X"
					end
					text = text .. inputname
				end
			end
			if entry.icons then
				for j, icon in ipairs(entry.icons) do
					if j > 1 then
						text = text .. " "
					end
					text = text .. "\27[" .. icon .. "]"
...
function widgetSystem.removeInputBar()
	if not private.inputBar then
		return
	end

	private.height = private.height + private.inputBar.height
	private.inputBar = nil

	-- deactivate slide
	local inputBarElement = getElement("inputBar", private.widgetsystem)
	goToSlide(inputBarElement, "inactive")
end
...
function widgetSystem.addHelpOverlay(frameElement, widgetID, x, y, width, height, standardbutton, backgroundcolspanwidth, overrideinfo)
	...
	elseif standardbutton then
		ffioverlayinfo = C.GetStandardButtonHelpOverlayInfo3(widgetID, standardbutton)
	else
		ffioverlayinfo = C.GetHelpOverlayInfo3(widgetID)
	end
	...
	if overlayinfo.id ~= "" then
		...
		if overlayinfo.text ~= "" then
			...
			if backgroundcolspanwidth and overlayinfo.usebackgroundspan then
				overlaywidth = backgroundcolspanwidth + 2 * private.scaledSizes.table_borderSize
			end
...
function widgetSystem.showFrameElementBorder(frameBorderElement, borderelementinfo)
	local x			= borderelementinfo.x
	local y			= borderelementinfo.y
	local width		= borderelementinfo.width
	local height	= borderelementinfo.height
	local linewidth = borderelementinfo.linewidth
	local offsets	= borderelementinfo.offsets

	local hasicon = borderelementinfo.icon.iconid ~= ""
	local hasicon2 = borderelementinfo.icon2.iconid ~= ""

	if hasicon and hasicon2 then
		goToSlide(frameBorderElement, "icon2")
	elseif hasicon or hasicon2 then
		goToSlide(frameBorderElement, "icon")
	else
		goToSlide(frameBorderElement, "normal")
	end
	-- position
	widgetSystem.setElementPosition(frameBorderElement, x + width / 2, y - height / 2, (x % 2 ~= 0), (y % 2 ~= 0))
	-- frame
	for _, element in ipairs(config.frameElementBorder.borderElements) do
		local anarkElement = getElement(element, frameBorderElement)
		if element == "border_left" then
			widgetSystem.setElementPosition(anarkElement, -(width + linewidth) / 2 - offsets.left, (offsets.top - offsets.bottom) / 2, (width + linewidth % 2 ~= 0), (height % 2 ~= 0))
			widgetSystem.setElementScale(anarkElement, linewidth / 100, (height + offsets.top + offsets.bottom) / 100)
		elseif element == "border_right" then
			widgetSystem.setElementPosition(anarkElement, (width + linewidth) / 2 + offsets.right, (offsets.top - offsets.bottom) / 2, (width + linewidth % 2 ~= 0), (height % 2 ~= 0))
			widgetSystem.setElementScale(anarkElement, linewidth / 100, (height + offsets.top + offsets.bottom) / 100)
		elseif element == "border_top" then
			widgetSystem.setElementPosition(anarkElement, (-offsets.left + offsets.right) / 2, (height + linewidth) / 2 + offsets.top, (width % 2 ~= 0), ((height + linewidth) % 2 ~= 0))
			widgetSystem.setElementScale(anarkElement, (width + 2 * linewidth + offsets.left + offsets.right) / 100, linewidth / 100)
		elseif element == "border_bottom" then
			widgetSystem.setElementPosition(anarkElement, (-offsets.left + offsets.right) / 2, -(height + linewidth) / 2 - offsets.bottom, (width % 2 ~= 0), ((height + linewidth) % 2 ~= 0))
			widgetSystem.setElementScale(anarkElement, (width + 2 * linewidth + offsets.left + offsets.right) / 100, linewidth / 100)
		end

		widgetSystem.setElementColorAlpha(getElement(element .. ".Material1119", frameBorderElement), borderelementinfo.color)
	end
	-- icon
	if hasicon then
		local iconElement = getElement("icon", frameBorderElement)
		widgetSystem.showFrameElementBorderIcon(iconElement, borderelementinfo.icon, borderelementinfo)
	end
	-- icon2
	if hasicon2 then
		local iconElement = getElement(hasicon and "icon2" or "icon", frameBorderElement)
		widgetSystem.showFrameElementBorderIcon(iconElement, borderelementinfo.icon2, borderelementinfo)
	end
end

function widgetSystem.showFrameElementBorderIcon(iconElement, iconinfo, borderelementinfo)
	local width		= borderelementinfo.width
	local height	= borderelementinfo.height
	local linewidth = borderelementinfo.linewidth
	local offsets	= borderelementinfo.offsets
	...
	local x, y = 0, 0
	if iconinfo.anchorid == "top_left" then
		x = -(width + 2 * linewidth) / 2 - offsets.left - iconinfo.width / 2
		y = (height + 2 * linewidth) / 2 + offsets.top + iconinfo.height / 2
	elseif iconinfo.anchorid == "top_left_inside" then
		x = -width / 2 - offsets.left + iconinfo.width / 2
		y = height / 2 + offsets.top - iconinfo.height / 2
	elseif iconinfo.anchorid == "top_right" then
		x = (width + 2 * linewidth) / 2 + offsets.right + iconinfo.width / 2
		y = (height + 2 * linewidth) / 2 + offsets.top + iconinfo.height / 2
	elseif iconinfo.anchorid == "top_right_inside" then
		x = width / 2 + offsets.right - iconinfo.width / 2
		y = height / 2 + offsets.top - iconinfo.height / 2
	elseif iconinfo.anchorid == "bottom_left" then
		x = -(width + 2 * linewidth) / 2 - offsets.left - iconinfo.width / 2
		y = -(height + 2 * linewidth) / 2 - offsets.bottom - iconinfo.height / 2
	elseif iconinfo.anchorid == "bottom_left_inside" then
		x = -width / 2 - offsets.left + iconinfo.width / 2
		y = -height / 2 - offsets.bottom + iconinfo.height / 2
	elseif iconinfo.anchorid == "bottom_right" then
		x = (width + 2 * linewidth) / 2 + offsets.right + iconinfo.width / 2
		y = -(height + 2 * linewidth) / 2 - offsets.bottom - iconinfo.height / 2
	elseif iconinfo.anchorid == "bottom_right_inside" then
		x = width / 2 + offsets.right - iconinfo.width / 2
		y = -height / 2 - offsets.bottom + iconinfo.height / 2
	end
	x = x + iconinfo.x
	...
	local material = getElement("icon", iconElement)
	SetIcon(getElement("icon", material), iconinfo.iconid, iconinfo.color.r, iconinfo.color.g, iconinfo.color.b, false, iconinfo.width, iconinfo.height)
	setAttribute(material, "opacity", iconinfo.color.a)
	setAttribute(material, "glowfactor", iconinfo.color.glow)
end

function widgetSystem.hideFrameElementBorder(frameBorderElement)
	goToSlide(frameBorderElement, "inactive")
	widgetSystem.returnElement("frameborder", frameBorderElement)
end
...
AddGlobalAccess("GetWidgetSystemSizes",					widgetSystem.getWidgetSystemSizes)
...
AddGlobalAccess("CreateInputBar",						widgetSystem.createInputBar)
AddGlobalAccess("UpdateInputBar",						widgetSystem.updateInputBar)
AddGlobalAccess("RemoveInputBar",						widgetSystem.removeInputBar)
...
function CallHotkeyScripts(action, active)
	local success, errorMessage
	for _, curFunction in ipairs(private.hotkeyScripts) do
		success, errorMessage = pcall(curFunction, action, active)
		if not success then
