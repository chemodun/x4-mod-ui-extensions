...
function menu.dropdownLoadout(_, loadoutid)
	if loadoutid ~= nil then
		...
		if menu.mode == "customgamestart" then
			-- no loadout crew definitions for custom gamestarts
			upgradeplan.crew = {}
		end
		if menu.usemacro then
			menu.captainSelected = true
...
function menu.onShowMenu(state)
	...
	elseif menu.mode == "upgrade" then
		...
		for i = #menu.selectableships, 1, -1 do
			...
			local uix_ownerfactionid = ffi.string(C.GetOwnerDetails(menu.selectableships[i]).factionID)
			local uix_ownerallowed = uix_ownerfactionid == "player"
			...
			if menu.uix_callbacks["onShowMenu_on_check_selectable_ship_owner"] then
				for uix_id, uix_callback in pairs(menu.uix_callbacks["onShowMenu_on_check_selectable_ship_owner"]) do
					local callbackresult = uix_callback({
						...
						ownerfactionid = uix_ownerfactionid,
						ownerallowed = uix_ownerallowed,
					})
					if callbackresult and callbackresult.ownerallowed then
						uix_ownerallowed = true
					end
				end
			end
			...
			elseif not uix_ownerallowed then
			-- alakeram end: callback
...
function menu.getDataAndDisplay(upgradeplan, crew, newedit, firsttime, noundo, settings)
	...
	if menu.uix_callbacks["getDataAndDisplay_on_assemble_possible_upgrades"] then
		for uix_id, uix_callback in pairs(menu.uix_callbacks["getDataAndDisplay_on_assemble_possible_upgrades"]) do
			...
			if callbackresult and callbackresult.upgradewares then
				menu.upgradewares = callbackresult.upgradewares
			end
		end
	end
	-- alakeram end: callback
		...
		if next(menu.groupedupgrades) then
										...
										if group[i] then
											...
											if menu.uix_callbacks["displaySlots_on_create_upgrade_button"] then
												for uix_id, uix_callback in pairs(menu.uix_callbacks["displaySlots_on_create_upgrade_button"]) do
													local callbackresult = uix_callback({
														macro = group[i].macro,
														plannedmacro = plandata.macro,
														upgradetype = upgradetype2.type,
														slot = menu.currentSlot,
														grouped = true,
														active = active,
														useable = useable,
														mouseovertext = mouseovertext,
													...
													if callbackresult then
														if callbackresult.active ~= nil then active = callbackresult.active end
			...
			elseif menu.upgradetypeMode == "settings" then
				...
				for i, entry in ipairs(types) do
					...
					if menu.object and (menu.object ~= 0) then
	...
	if id ~= "" then
		...
		elseif entry.type == "slot" then
			...
			elseif (entry.type == "float") or (entry.type == "double") then
				value = Helper.formatLoadoutStat(loadoutstats[id], entry.accuracy)
			end
...
function menu.displayEquipmentInfoFrame()
	...
	if menu.equipmentData.mouse or menu.equipmentData.keyboard then
		Helper.createEquipmentInfoContext(menu, menu.equipmentInfoFrame, menu.object, menu.macro, menu.upgradeplan)
	end
