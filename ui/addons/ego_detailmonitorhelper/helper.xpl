...
function Helper.createEquipmentInfoContext(menu, frame, object, objectmacro, upgradeplan)
	...
	local plandata = upgradeplan[upgradetype.type][data.software and data.slot or menu.currentSlot]
	...
	if data.macro and (data.macro ~= "") then
		...
		if object ~= 0 then
			iscapship = C.IsComponentClass(object, "ship_l") or C.IsComponentClass(object, "ship_xl")
		elseif objectmacro ~= "" then
			iscapship = IsMacroClass(objectmacro, "ship_l") or IsMacroClass(objectmacro, "ship_xl")
		end
		...
		local shipmacro = (objectmacro ~= "") and objectmacro or GetComponentData(ConvertStringToLuaID(tostring(object)), "macro")
		local shipmodifiers = {}
		...
		local loc_upgradeplan = Helper.tableCopy(upgradeplan, 3)
		if upgradetype.mergeslots then
			for i, slotdata in ipairs(menu.slots[slotupgradetype]) do
				loc_upgradeplan[slotupgradetype][i] = { macro = macro, ammomacro = "", weaponmode = "" }
			end
		else
			loc_upgradeplan[slotupgradetype][menu.currentSlot] = { macro = macro, ammomacro = "", weaponmode = "" }
		end
		...
		if (infolibrary == "enginetypes") or (infolibrary == "thrustertypes") then
			...
			if object == 0 then
				ffiloadoutstats = Helper.callLoadoutFunction(loc_upgradeplan, nil, function (loadout, _) return C.GetLoadoutStatistics5(0, objectmacro, loadout) end)
			else
				ffiloadoutstats = Helper.callLoadoutFunction(loc_upgradeplan, nil, function (loadout, _) return C.GetLoadoutStatistics5(object, "", loadout) end)
			end
		...
		if infolibrary == "enginetypes" then
			for _, entry in ipairs(Helper.equipmentStats.engine) do
				rowparent = Helper.addEquipmentInfoRow(backgroundrowgroup, rowparent, entry, loadoutstats[entry.id], menu.loadoutstats[entry.id])
			end

			if iscapship then
				Helper.addEquipmentInfoRow(backgroundrowgroup, backgroundrowgroup, { name = ReadText(1001, 9083), unit = ReadText(1001, 118), standalone = true }, librarydata.hull, installedlibrarydata and installedlibrarydata.hull or 0)
			end
		elseif (infolibrary == "thrustertypes") then
			for _, entry in ipairs(Helper.equipmentStats.thruster) do
				rowparent = Helper.addEquipmentInfoRow(backgroundrowgroup, rowparent, entry, loadoutstats[entry.id], menu.loadoutstats[entry.id])
			end
		else
			for _, entry in ipairs(Helper.equipmentStats[infolibrary]) do
				if (entry.statcondition == nil) or (Helper.checkStatCondition(statconditions, entry.statcondition) == (not entry.negatestatcondition)) then
					if entry.id then
						local value, display = Helper.getEquipmentStatValue(shipmodifiers, librarydata, entry)
						local installedvalue, installeddisplay = Helper.getEquipmentStatValue(shipmodifiers, installedlibrarydata, entry)
						if display or installeddisplay then
							rowparent = Helper.addEquipmentInfoRow(backgroundrowgroup, rowparent, entry, value, installedvalue, entry.iconid and librarydata[entry.iconid] or nil)
						end
					else
						rowparent = Helper.addEquipmentInfoRow(backgroundrowgroup, rowparent, entry)
					end
				end
			end
		end
...
function Helper.getEquipmentStatValue(shipmodifiers, librarydata, entry)
	local value = 0
...
function Helper.addEquipmentInfoRow(ftable, rowparent, entry, value, installedvalue, iconvalue)
	if not value then
		...
		if entry.userangeformat then
			textvalue = Helper.formatRange(value)
		elseif entry.accuracy then
			textvalue = Helper.formatLoadoutStat(textvalue, entry.accuracy)
		...
		if type(textvalue) == "number" then
			if entry.usenaturalunits then
				if textvalue >= 0.01 * Helper.speedoflight then
					textvalue = math.floor(textvalue * 100 / Helper.speedoflight + 0.5) / 100 .. " " .. ReadText(1001, 124)
					unit = nil
				else
...
function Helper.formatLoadoutStat(value, accuracy)
...
function Helper.formatRange(range)
	return string.format("%." .. ((range > 1000) and 1 or 3) .. "f", Helper.round(range / 1000, (range > 1000) and 1 or 3))
end

function Helper.checkStatCondition(statconditions, statcondition)
